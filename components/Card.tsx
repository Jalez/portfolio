...
const Card: React.FC<CardProps> = ({ 
  ...
  return (
    <div 
      ...
      } flex flex-col h-full group ${animatedBorder ? 'hover-border-animation' : ''} ${onClick ? 'cursor-pointer' : ''} ${className}`}
      onClick={onClick}
    >
      ...
      {/* Card content */}
      <div className={`relative bg-theme-secondary ${animatedBorder ? 'm-0.5' : ''} ${paddingClasses[padding]} rounded-lg shadow-xl flex flex-col h-full border border-theme group-hover:border-transparent transition-all duration-300 ${variantClasses[variant]}`}>
        {children}
      </div>
    </div>
  );
